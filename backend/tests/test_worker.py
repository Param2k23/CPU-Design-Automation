...
from app.queue.redis_queue import enqueue_job, dequeue_job, QUEUE_NAME

def test_enqueue_dequeue():
    # Mock redis client
    with patch("app.queue.redis_queue.redis_client") as mock_redis:
        # Test enqueue
        enqueue_job("test_job_id", priority="high")
        mock_redis.lpush.assert_called_once_with(QUEUE_NAME, json.dumps({"job_id": "test_job_id", "priority": "high"}))

        # Test dequeue with item
        mock_redis.brpop.return_value = (QUEUE_NAME, json.dumps({"job_id": "test_job_id", "priority": "high"}))
        job = dequeue_job(timeout=1)
        assert job == {"job_id": "test_job_id", "priority": "high"}
